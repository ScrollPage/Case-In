    ...
    if "events" in call.data:
        try:
            ...
            queryset = queryset.filter(
                datetime__gte=datetime.now(), datetime__lte=end_date
            )
            if queryset:
        ...
        if event == "Миссия.":
            ...
            bot.send_photo(
                ...
                settings.AWS_S3_CUSTOM_DOMAIN + f"/rosatom_bot/rosatom1.png",
            )
            bot.send_photo(
                call.message.chat.id,
                settings.AWS_S3_CUSTOM_DOMAIN + f"/rosatom_bot/rosatom2.png",
            )
            ...
            l = ["Технологиии.", "Суперкомпьютеры.", "Учет результатов."]
        ...
        try:
            ...
            if event == "Моя должность.":
                bot.send_message(
                    call.message.chat.id,
                    f"Ваша должность: {code.user.info.position}",
                    reply_markup=menu,
                )
                ...
                if code.user.mentor:
                    bot.send_message(
                        call.message.chat.id,
                        f"Ваш наставник {code.user.mentor}",
                        reply_markup=menu,
                    )
                else:
                    bot.send_message(
                ...
                if departs:
                    res = "".join(
                        f"Отдел: {dapert}. Глава отдела: {depart.admin.get_full_name()}\n"
                    ...
                    bot.send_message(
                        call.message.chat.id,
                        "Вы не состоите ни в одном из отделов.",
                        reply_markup=menu,
                    )
...
def get_various_messages(message):
    ...
    if message.text == "Авторизация":
        ...
        try:
            ...
            user = code.user
            menu = get_menu(message)
            l = ["на следующий день", "на следующую неделю", "на следующий месяц"]
