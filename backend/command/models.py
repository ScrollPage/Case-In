...
class Command(models.Model):
    ...
    whitelist = models.ManyToManyField(
        Initiative, verbose_name="Белый список на скачивание", related_name="+"
    )
...
class CommandInfo(models.Model):
    ...
    command = models.OneToOneField(
        Command, verbose_name="Команда", on_delete=models.CASCADE, related_name="info"
    )
...
class JoinRequest(models.Model):
    ...
    initiative = models.ForeignKey(
        Initiative, verbose_name="Отправитель", on_delete=models.CASCADE, null=True
    )
...
class AccessRequest(models.Model):
    ...
    initiative = models.ForeignKey(
        Initiative, verbose_name="Отправитель", on_delete=models.CASCADE
    )
...
def send_join_note(sender, instance=None, **kwargs):
    ...
    Notification.objects.create(
        note=1, command=instance.command, initiative=instance.initiative
    )
...
def send_access_note(sender, instance=None, **kwargs):
    ...
    Notification.objects.create(
        note=2, command=instance.command, initiative=instance.initiative
    )
    pusher_client.trigger(
        f"notifications{instance.initiative.id}",
        "access-request",
