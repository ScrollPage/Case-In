...
class DepartmentInfo(models.Model):
    ...
    depart = models.OneToOneField(
        Department, verbose_name="Отдел", related_name="info", on_delete=models.CASCADE
    )
...
def create_instances(sender, instance=None, created=False, **kwargs):
    ...
    if created:
        ...
        Chat.objects.create(
            depart=instance, is_chat=False, name=f"Чат отдела {instance}"
        )
