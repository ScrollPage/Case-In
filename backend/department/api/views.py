...
class DepartmentViewSet(SPFModelViewSet):
    ...
    def post(self, request, *args, **kwargs):
        ...
        posts = (
            posts.annotate(num_likes=Count("likes", distinct=True))
            .annotate(
                is_liked=Count(
                    "likes", filter=Q(likes__user=request.user), distinct=True
                )
            )
    ...
    def calendlytask(self, request, *args, **kwargs):
        ...
        tasks = depart.calendly_tasks.all().annotate(
            going=Count(
                "confirmations",
    ...
    def get_queryset(self):
        ...
        if self.action != "retrieve" and self.action != "list":
            return queryset
        return (
            queryset.annotate(num_workers=Count("workers", distinct=True))
            .annotate(
                joined=Exists(
                    DepMembership.objects.filter(
                        depart=OuterRef("id"), user=self.request.user
                    )
                )
            )
